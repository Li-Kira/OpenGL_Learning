...
const char* WINDOW_TITLE = "15b_GeometryShader_Exploding";
...
int main() {
    ...
    Shader shader("res/shader/15b_exploding.vert", "res/shader/15b_exploding.frag", "res/shader/15b_exploding.geom");
    std::string modelPath = "res/model/nanosuit/nanosuit.obj";
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::Checkbox("Auto Time", &useAutoTime);
        if (useAutoTime) {
            ImGui::SliderFloat("Time Speed", &timeSpeed, 0.0f, 5.0f);
            timeValue = static_cast<float>(glfwGetTime()) * timeSpeed;
        } else {
            ImGui::SliderFloat("Time Value", &timeValue, 0.0f, 10.0f);
        }
        ImGui::Text("Current Time: %.2f", timeValue);
        ImGui::End();
