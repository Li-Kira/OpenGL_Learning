...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "07_ModelLoading", NULL, NULL);
    if (window == NULL)
    {
    ...
    Shader shader("res/shader/07_modelLoading.vert", "res/shader/07_modelLoading.frag");
...
void ShowFPS(GLFWwindow* window, float fps) {
    std::string title = "07_ModelLoading - FPS: " + std::to_string(static_cast<int>(fps));
