...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "06_MultipleLights", NULL, NULL);
    if (window == NULL)
    {
    ...
    Shader lightingShader("res/shader/06_multiLight.vert", "res/shader/06_multiLight.frag");
    ...
    unsigned int texture1 = TextureUtils::loadTexture("res/texture/container2.png");
    unsigned int texture2 = TextureUtils::loadTexture("res/texture/container2_specular.png");
    lightingShader.use(); 
        ...
        for (unsigned int i = 0; i < 4; i++)
         {
             ...
             lightCubeShader.setMat4("u_ModelMatrix", model);
             glDrawArrays(GL_TRIANGLES, 0, 36);
         }
...
void ShowFPS(GLFWwindow* window, float fps) {
    std::string title = "06_MultipleLights - FPS: " + std::to_string(static_cast<int>(fps));
