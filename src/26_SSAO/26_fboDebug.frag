...
void main()
{
    ...
    if (debugMode <= 1) {
        vec3 resultColor = texture(sceneTex, texCoord).rgb;
        ...
        vec4 albedoSpec = texture(ssaoColorBuffer, texCoord);
        FragColor = albedoSpec;
    }
    else if (debugMode == 5) {
        vec4 ssaoColor = texture(ssaoColorBufferBlur, texCoord);
        FragColor = ssaoColor;
    }
    else if (debugMode == 6) {
        ...
        if (TexCoords.x < 0.5 && TexCoords.y >= 0.5) {
            ...
            vec4 albedoSpec = texture(ssaoColorBuffer, quadrantCoord);
