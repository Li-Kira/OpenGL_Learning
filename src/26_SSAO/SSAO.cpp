...
const char* WINDOW_TITLE = "26_SSAO";
...
int main() {
    ...
    // Shader info
    // -------------
    Shader geometryPassShader("../26_ssaoGeometry.vert", "../26_ssaoGeometry.frag");
    Shader lightingShader("../26_ssao.vert", "../26_ssaoLighting.frag");
    Shader ssaoShader("../26_ssao.vert", "../26_ssao.frag");
    Shader ssaoBlurShader("../26_ssao.vert", "../26_ssaoBlur.frag");
    Shader debugShader("../26_fboDebug.vert", "../26_fboDebug.frag");
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, fbWidth, fbHeight, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, ssaoColorBuffer, 0);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        Logger::log(Logger::Level::Error, "SSAO Framebuffer not complete!");
    // and blur stage
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        Logger::log(Logger::Level::Error, "SSAO Blur Framebuffer not complete!");
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    // Debug color buffer
    glGenTextures(1, &debugBuffer);
    glBindTexture(GL_TEXTURE_2D, debugBuffer);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, fbWidth, fbHeight, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, debugBuffer, 0);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        Logger::log(Logger::Level::Error, "Debug Framebuffer not complete!");
    ...
    debugShader.use();
    debugShader.setInt("sceneTex", 0);
    debugShader.setInt("gPosition", 1);
    debugShader.setInt("gNormal", 2);
    debugShader.setInt("ssaoColorBuffer", 3);
    debugShader.setInt("ssaoColorBufferBlur", 4);
    ...
    // Uniform Settings
    int debugMode = 0;  // 0: no debug, 1: oriColor, 2: gPosition, 3: gNormal, 4: Just Shadow, 4: Blured Shadow
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::SliderFloat("Bias", &bias, 0.0f, 5.0f);
        ImGui::End();
        ...
        ssaoShader.setVec2("u_pixelSize", glm::vec2(float(fbWidth), float(fbHeight)));
        ...
        lightingShader.setInt("debugMode", debugMode);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, gPosition);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, gNormal);
