...
const char* WINDOW_TITLE = "12_FrameBuffer";
...
int main() {
    ...
    Shader shader("res/shader/12_frameBuffer.vert", "res/shader/12_frameBuffer.frag");
    Shader screenShader("res/shader/12_frameBufferScreen.vert", "res/shader/12_frameBufferScreen.frag");

    unsigned int cubeTexture = TextureUtils::loadTexture("res/texture/marble.jpg", false);
    unsigned int floorTexture = TextureUtils::loadTexture("res/texture/metal.png", false);

    shader.use(); 
    shader.setInt("u_InputTex", 0);

    screenShader.use();
    screenShader.setInt("u_InputTex", 0);

    // framebuffer configuration
    // -------------------------
    // resolution adaption on Mac
    int fbWidth, fbHeight;
    glfwGetFramebufferSize(window, &fbWidth, &fbHeight);
    // Logger::log(Logger::Level::Info, "SCR_WIDTH: " , SCR_WIDTH);
    // Logger::log(Logger::Level::Info, "SCR_HEIGHT: " , SCR_HEIGHT);
    // Logger::log(Logger::Level::Info, "fbWidth: " , fbWidth);
    // Logger::log(Logger::Level::Info, "fbHeight: " , fbHeight);
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        Logger::log(Logger::Level::Error, "ERROR::FRAMEBUFFER:: Framebuffer is not complete!");
